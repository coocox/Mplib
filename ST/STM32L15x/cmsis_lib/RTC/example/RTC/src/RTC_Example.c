...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
uint32_t RTC_BKP_DR[RTC_BKP_DR_NUMBER] = { RTC_BKP_DR0, RTC_BKP_DR1,
...
void WriteToRTC_BKP_DR(uint32_t FirstRTCBackupData) {
	...
	for (index = 0; index < RTC_BKP_DR_NUMBER; index++) {
		...
		RTC_WriteBackupRegister(RTC_BKP_DR[index],
				FirstRTCBackupData + (index * 0x5A));
	}
}
...
uint32_t CheckRTC_BKP_DR(uint32_t FirstRTCBackupData) {
	...
	for (index = 0; index < RTC_BKP_DR_NUMBER; index++) {
		...
		if (RTC_ReadBackupRegister(RTC_BKP_DR[index])
				!= (FirstRTCBackupData + (index * 0x5A))) {
			return (index + 1);
		}
	}
