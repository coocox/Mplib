...
volatile TestStatus EraseStatus = FAILED, TransferStatus1 = FAILED,
...
void SD_EraseTest(void) {
	...
	if (Status == SD_OK) {
		Status = SD_ReadMultiBlocks(Buffer_MultiBlock_Rx, 0x00, BLOCK_SIZE,
				NUMBER_OF_BLOCKS);
...
void SD_MultiBlockTest(void) {
	...
	if (Status == SD_OK) {
		...
		Status = SD_WriteMultiBlocks(Buffer_MultiBlock_Tx, 0x00, BLOCK_SIZE,
				NUMBER_OF_BLOCKS);
		/* Check if the Transfer is finished */
		Status = SD_WaitWriteOperation();
		while (SD_GetStatus() != SD_TRANSFER_OK)
	...
	/* Check the correctness of written data */
	if (Status == SD_OK) {
		TransferStatus2 = Buffercmp(Buffer_MultiBlock_Tx, Buffer_MultiBlock_Rx,
				MULTI_BUFFER_SIZE);
	}
