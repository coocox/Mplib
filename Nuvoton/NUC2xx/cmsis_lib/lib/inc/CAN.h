...
/** @addtogroup CAN_FUNC   CAN Device Function Interface
  * @{
  */
...
/*---------------------------------------------------------------------------------------------------------*/
#define CAN_MAJOR_NUM	3
#define CAN_MINOR_NUM	01
#define CAN_BUILD_NUM	001

/*---------------------------------------------------------------------------------------------------------*/
/*  Version define with SysInfra				                                                           */
/*---------------------------------------------------------------------------------------------------------*/
#define CAN_VERSION_NUM     _SYSINFRA_VERSION(CAN_MAJOR_NUM,CAN_MINOR_NUM,CAN_BUILD_NUM) 
								 
/*---------------------------------------------------------------------------------------------------------*/
/*  Define Error Code									                                                   */
/*---------------------------------------------------------------------------------------------------------*/
// E_CAN_ERR_TIMEOUT  				Time out
// E_CAN_ERR_PORT_INVALID		 	Wrong port
// E_CAN_ARGUMENT                	Wrong Argument (Wrong CAN Port)
// E_CAN_ERR_BITRATE                	Wrong bit rate configure 
// E_CAN_NO_PENDING_MSG              No pending error to recevie 
// E_CAN_NO_USEFUL_INTERFACE         No useful interface
// E_CAN_NO_RECEIVED_NEWDATA         No received new data
#define E_CAN_ERR_TIMEOUT   			-1
#define E_CAN_ERR_PORT_INVALID	  	-2
#define E_CAN_ERR_ARGUMENT         	-3
#define E_CAN_ERR_BITRATE          	-4
#define E_CAN_NO_PENDING_MSG        	-5
#define E_CAN_NO_USEFUL_INTERFACE   	-6
#define E_CAN_NO_RECEIVED_NEWDATA   	-7
...
void CAN_EnterTestMode(uint8_t TestMask);
void CAN_LeaveTestMode(void);
void CAN_ResetIF(uint8_t number);
...
int32_t CAN_ResetMsgObj(uint8_t msgobj);
int32_t CAN_Open(uint32_t u32bitrate);
int32_t CAN_WaitEndOfTx(void);
int32_t CAN_ClrIntPnd(uint8_t msgobj);
...
int32_t CAN_ReadMsgObj(uint8_t msgobj, uint8_t release, STR_CANMSG_T* pCanMsg);
...
int32_t CAN_GetVersion(void);

uint32_t CAN_IsNewDataReceived(uint8_t msgobj);
uint32_t CAN_SetTxRqst(uint8_t u8msgobj);
uint32_t CAN_IsObjectValid(uint8_t msgobj);
uint32_t CAN_IsTxRqstPending(uint8_t msgobj);
...
/**
  * @} End of CAN Device Function Interface
  */ 
