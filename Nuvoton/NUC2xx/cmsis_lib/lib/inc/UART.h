...
static __INLINE void UART_SetRTS_TrgLev(UART_T* UART,uint32_t eActLevel,uint32_t eTRG_Level)
{
 ...
 *
 */ 
 static __INLINE void UART_ResetIP(UART_T* UART)
{
...
#define _UART_CLEAR_BREAK(UART)           ((UART)->LCR &= ~UART_LCR_BCB_Msk)
 ...
 static __INLINE int32_t UART_Write(UART_T *UART,uint8_t *pu8TxBuf,uint32_t u32WriteBytes)
{
...
static __INLINE int32_t UART_Read(UART_T *UART,uint8_t *pu8RxBuf, uint32_t u32ReadBytes)
{
    uint32_t u32Count, u32delayno;
