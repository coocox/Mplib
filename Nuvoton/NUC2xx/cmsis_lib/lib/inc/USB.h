...
/** @addtogroup USBD_FUNC   USB Device Function Interface
  * @{
  */
...
//E_USBD_NULL_POINTER             Callback function is NULL.
//E_USBD_INVALID_EP_NUM           EndPoint number error.
//E_USBD_SIZE_TOO_LONG            Data size is too long.
//E_USBD_VALUE_INVALID            Input argument is invalid.  

#define E_USBD_NULL_POINTER       -1
#define E_USBD_INVALID_EP_NUM     -2
#define E_USBD_SIZE_TOO_LONG      -3
#define E_USBD_VALUE_INVALID      -4
...
#define USBD_SRAM_BASE       (USBD_BASE + 0x100)
...
#define _USBD_DATA0(Data)           ((Data) & 0x37F)
#define _USBD_DATA1(Data)           ((Data) | 0x80)
#define _USBD_ENABLE_MISC_INT(u32Flags)   outp32(USBD_INTEN, (u32Flags) & (INTEN_WAKEUP + INTEN_FLDET + INTEN_USB + INTEN_BUS))

#define _USBD_ENABLE_WAKEUP()       USBD->INTEN |= USBD_INTEN_WAKEUP_EN_Msk
#define _USBD_DISABLE_WAKEUP()      USBD->INTEN &= (~USBD_INTEN_WAKEUP_EN_Msk)
#define _USBD_ENABLE_WAKEUP_INT()   USBD->INTEN |= USBD_INTEN_WAKEUP_IE_Msk
#define _USBD_DISABLE_WAKEUP_INT()  USBD->INTEN &= (~USBD_INTEN_WAKEUP_IE_Msk)
#define _USBD_ENABLE_FLDET_INT()    USBD->INTEN |= USBD_INTEN_FLDET_IE_Msk
#define _USBD_DISABLE_FLDET_INT()   USBD->INTEN &= (~USBD_INTEN_FLDET_IE_Msk)
#define _USBD_ENABLE_USB_INT()      USBD->INTEN |= USBD_INTEN_USB_IE_Msk
#define _USBD_DISABLE_USB_INT()     USBD->INTEN &= (~USBD_INTEN_USB_IE_Msk)
#define _USBD_ENABLE_BUS_INT()      USBD->INTEN |= USBD_INTEN_BUS_IE_Msk
#define _USBD_DISABLE_BUS_INT()     USBD->INTEN &= (~USBD_INTEN_BUS_IE_Msk)

/* Respond EPx (x = 0 ~ 5) STALL packet */
#define _USBD_CLEAR_EP_READY_AND_TRIG_STALL(u32EpId)      outp32(USBD_CFGP0+(0x10*(u32EpId)), (inp32(USBD_CFGP0+(0x10*(u32EpId))) | (CFGP_SSTALL | CFGP_CLRRDY)))
    
/* Clear EPx (x = 0 ~ 5) In/Out ready */
#define _USBD_CLEAR_EP_READY(u32EpId)                     outp32(USBD_CFGP0+(0x10*(u32EpId)), (inp32(USBD_CFGP0+(0x10*(u32EpId))) | CFGP_CLRRDY))

/* Specify USB buffer for SETUP packet */
#define _USBD_SET_SETUP_BUF(u32BufAddr)                   outp32(USBD_BUFSEG, (u32BufAddr) - USBD_SRAM_BASE)

/* Specify USB buffer for EPx (x = 0 ~ 5) */
#define _USBD_SET_EP_BUF(u32EpId, u32BufAddr)             outp32(USBD_BUFSEG0+(0x10 * (u32EpId)), (u32BufAddr) - USBD_SRAM_BASE)
        
/* Trigger next transaction for EPx (x = 0 ~ 5) */
#define _USBD_TRIG_EP(u32EpId, u32TrigSize)               outp32((USBD_MXPLD0+(0x10*(u32EpId))), (u32TrigSize))

/* Set toggle bit for EPx (x = 0 ~ 5) */
#define _USBD_SET_EP_TOG_BIT(u32EpId, bData0)             outp32(USBD_CFG0+(0x10*(u32EpId)), bData0 ? _USBD_DATA0(inp32(USBD_CFG0+(0x10*(u32EpId)))) : _USBD_DATA1(inp32(USBD_CFG0+(0x10*(u32EpId)))))

/* Get Data Out size for EPx (x = 0 ~ 5) */
#define _USBD_GET_EP_DATA_SIZE(u32EpId)                   inp8(USBD_MXPLD0 + (0x10*(u32EpId)))

#define _USBD_SET_EVENT_FLAG(u32Data)                     outp32((&USBD->INTSTS), (u32Data))  

#define _USBD_GET_EVENT_FLAG()                            inp32((&USBD->INTSTS))

/* Clear EPx (x = 0 ~ 5) Force device to response STALL */
#define _USBD_CLEAR_EP_STALL(u32EpId)                     outp32(USBD_CFGP0+(0x10 * (u32EpId)), inp32(USBD_CFGP0+(0x10 * (u32EpId))) & ~CFGP_SSTALL)
    
/* Trigger EPx (x = 0 ~ 5) Force device to response STALL */
#define _USBD_TRIG_EP_STALL(u32EpId)                      outp32(USBD_CFGP0+(0x10*(u32EpId)), inp32(USBD_CFGP0+(0x10*(u32EpId))) | CFGP_SSTALL)

/* Clear EPx (x = 0 ~ 5) Specify Data 0 or 1 after IN token toggle automatically after host ACK */
#define _USBD_CLEAR_EP_DSQ_SYNC(u32EpId)                  outp32(USBD_CFG0+(0x10*(u32EpId)), inp32(USBD_CFG0+(0x10*(u32EpId))) & ~CFG_DSQ_SYNC)

/* Configure Set CFGx (x = 0 ~ 5) */
#define _USBD_SET_CFG(u32CFGNum,u32Data)                  outp32(USBD_CFG0+(0x10*(u32CFGNum)), (u32Data) & 0x03FF)

/* Configure Get CFGx (x = 0 ~ 5) */    
#define _USBD_GET_CFG(u32CFGNum)                          (inp32(USBD_CFG0 + (u32CFGNum) * 0x10) & 0x03FF)

/* Set Function Address */
#define _USBD_SET_FADDR(u32Addr)                          (USBD->FADDR = u32Addr)

/* Get Function Address */  
#define _USBD_GET_FADDR()                                 (USBD->FADDR)

/* Get System states */
#define _USBD_GET_EPSTS()                                 inp32((&USBD->EPSTS))

/* Configure Set CFGPx (x = 0 ~ 5) */
#define _USBD_SET_CFGP(u8CFGPNum, u32Data)                outp32(USBD_CFGP0+(0x10 * (u8CFGPNum)), (u32Data) & 0x03)

#define _USBD_SET_CFG_EP0(u32Data)                        outp32(USBD_CFGP0, (u32Data) & 0x03)

#define _USBD_SET_CFG_EP1(u32Data)                        outp32(USBD_CFGP1, (u32Data) & 0x03)

#define _USBD_SET_CFG_EP2(u32Data)                        outp32(USBD_CFGP2, (u32Data) & 0x03)

#define _USBD_SET_CFG_EP3(u32Data)                        outp32(USBD_CFGP3, (u32Data) & 0x03)

#define _USBD_SET_CFG_EP4(u32Data)                        outp32(USBD_CFGP4, (u32Data) & 0x03)

#define _USBD_SET_CFG_EP5(u32Data)                        outp32(USBD_CFGP5, (u32Data) & 0x03)

/* Configure Get CFGPx (x = 0 ~ 5) */
#define _USBD_GET_CFGP(u32CFGPNum)                        (inp32(USBD_CFGP0+(0x10 * (u32CFGPNum))) & 0x03)
 
/* Enable USB , PHY and use remote wake-up */
#define _USBD_ENABLE_USB()                                outp32((&USBD->ATTR), 0x7D0)

/* Disable USB, PHY and use remote wake-up */
#define _USBD_DISABLE_USB()                               outp32((&USBD->ATTR), 0x760)

/* Disable PHY and don't use remote wake-up */
#define _USBD_DISABLE_PHY()                               outp32((&USBD->ATTR), 0x7C0)

#define _USBD_ENABLE_SE0()                                (USBD->DRVSE0 = 1)

#define _USBD_DISABLE_SE0()                               (USBD->DRVSE0 = 0)
...
#define USB_BUF_SETUP                   USBD_GetSetupBuffer()

#define USBD_EVENT_FLAG_ATTACH        0x00000001
#define USBD_EVENT_FLAG_DETACH        0x00000002
#define USBD_EVENT_FLAG_BUS_RESET     0x00000004
#define USBD_EVENT_FLAG_BUS_SUSPEND   0x00000008
#define USBD_EVENT_FLAG_BUS_RESUME    0x00000010
#define USBD_EVENT_FLAG_SETUP         0x00000020
...
{
    void * device;
    PFN_USBD_CALLBACK pfnStart;       /* The callback function when set configuration or set interface */
...
typedef struct
{
    ...
    S_USBD_EP_CTRL sEpCrl[MAX_EP_ID+1];
    int32_t  eUsbState;           /* E_USBD_STATE */
...
extern PFN_USBD_INTCALLBACK g_FnIntCallBack;  /* Interrupt call back  */
...
/*---------------------------------------------------------------------------------------------------------*/
/* Define USBD READ/WRITE USB BUFFER                                                                     */
/*---------------------------------------------------------------------------------------------------------*/
#define _USBD_READ_USB_BUF(UsbBuf, Mem, Len) \
    memcpy ((void *) (Mem), (void *) (UsbBuf), (Len))

#define _USBD_WRITE_USB_BUF(UsbBuf, Mem, Len) \
...
int32_t  USBD_IsSelfPowerEnabled (void);          /* TRUE : Self-Powereded, FALSE : BUS-Powereded */
void     USBD_EnableRemoteWakeup(void);
void     USBD_DisableRemoteWakeup(void);
int32_t  USBD_IsRemoteWakeupEnabled (void);       /* TRUE : Support RemoteWakeup, FALSE : not support */
int32_t  USBD_SetMaxPower(uint32_t u32MaxPower);  /* Maximum MaxPower 0xFA (500mA), default is 0x32 (100mA) */
int32_t  USBD_GetMaxPower(void);                  /* Maximum MaxPower 0xFA (500mA), default is 0x32 (100mA)   */
...
/**
  * @} End of USB Device Function Interface
  */ 
