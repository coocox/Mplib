 ...
 * $Revision: 9 $
 * $Date: 14/01/09 1:48p $
 * @brief    NUC400 series I2C driver source file
 *
 * @note
...
void I2C_Close(I2C_T *i2c)
{
    ...
    if((uint32_t)i2c == I2C0_BASE) {
        ...
        SYS->IPRSTC2 |= SYS_IPRSTC2_I2C2_RST_Msk;
        SYS->IPRSTC2 &= ~SYS_IPRSTC2_I2C2_RST_Msk;
    } else {
        SYS->IPRSTC2 |= SYS_IPRSTC2_I2C3_RST_Msk;
        SYS->IPRSTC2 &= ~SYS_IPRSTC2_I2C3_RST_Msk;
    }
...
{
    uint32_t u32Reg = 0;
    uint32_t u32Val = i2c->I2CON & ~(I2C_STA | I2C_STO | I2C_AA);
...
{
    i2c->I2CWKCON |= I2C_I2CWKCON_WKUPEN_Msk;
}
