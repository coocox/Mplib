 ...
 * $Revision: 6 $
 * $Date: 13/11/10 9:32a $
 * @brief    NUC400 series UART driver source file
 *
 * @note
...
void UART_Open(UART_T* uart, uint32_t u32baudrate)
{
    ...
    u32Clk = (u32ClkTbl[u8UartClkSrcSel]) / (((CLK->CLKDIV0 & CLK_CLKDIV0_UART_N_Msk) >> CLK_CLKDIV0_UART_N_Pos) + 1);
...
uint32_t UART_Read(UART_T* uart, uint8_t *pu8RxBuf, uint32_t u32ReadBytes)
{
    ...
    for(u32Count=0; u32Count < u32ReadBytes; u32Count++) {
        if(uart->FSR & UART_FSR_RX_EMPTY_Msk) { /* Check RX empty => failed */
            return u32Count;
        }
...
uint32_t UART_Write(UART_T* uart, uint8_t *pu8TxBuf, uint32_t u32WriteBytes)
{
    ...
    {
        if(uart->FSR & UART_FSR_TX_FULL_Msk)
