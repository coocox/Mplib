 ...
 * @version  V1.00
 * $Revision: 6 $
 * $Date: 14/01/03 11:59p $
 * @brief    NUC400 series EMAC driver source file
 *
 * @note
...
/** @addtogroup NUC400_EMAC_EXPORTED_TYPEDEF EMAC Exported Type Defines
  @{
*/
...
/*@}*/ /* end of group NUC400_EMAC_EXPORTED_TYPEDEF */
...
static uint32_t u32CurrentTxDesc, u32NextTxDesc, u32CurrentRxDesc; // These variables are share between driver and IRQ handler
...
/**
  ...
  * @return None
  */
#define EMAC_TRIGGER_RX() do{EMAC->RSDR = 0;}while(0)
...
/**
  ...
  * @return None
  */
#define EMAC_TRIGGER_TX() do{EMAC->TSDR = 0;}while(0)
...
static void EMAC_MdioWrite(uint32_t u32Reg, uint32_t u32Addr, uint32_t u32Data)
{
    ...
    EMAC->MIIDA = u32Reg | (u32Addr << 8) | EMAC_MIIDA_BUSY_Msk | EMAC_MIIDA_WRITE_Msk;
...
static uint32_t EMAC_MdioRead(uint32_t u32Reg, uint32_t u32Addr)
{
    ...
    EMAC->MIIDA = u32Reg | (u32Addr << 8) | EMAC_MIIDA_BUSY_Msk;
...
/**
  ...
  * @note This API configures EMAC to receive all broadcast and multicast packets, bu could configure to other settings with
...
uint32_t EMAC_RecvPkt(uint8_t *pu8Data, uint32_t *pu32Size)
{
    ...
    reg = EMAC->MISTA;
    EMAC->MISTA = reg | (EMAC_MISTA_MPR_Msk |
                         EMAC_MISTA_CFR_Msk |
                         EMAC_MISTA_RXBERR_Msk|
                         EMAC_MISTA_RDU_Msk|
                         EMAC_MISTA_DENI_Msk|
                         EMAC_MISTA_DFOI_Msk|
                         EMAC_MISTA_MMP_Msk|
                         ...
                         EMAC_MISTA_RXOV_Msk|
                         EMAC_MISTA_CRCE_Msk|
                         EMAC_MISTA_RXINTR_Msk);

    if (reg & EMAC_MIEN_ENRXBERR_Msk) {
...
uint32_t EMAC_SendPktDone(void)
{
    ...
    EMAC->MISTA = reg & (EMAC_MISTA_TXBERR_Msk |
                         EMAC_MISTA_TDU_Msk |
                         EMAC_MISTA_LC_Msk |
                         EMAC_MISTA_TXABT_Msk |
                         EMAC_MISTA_NCS_Msk |
                         EMAC_MISTA_EXDEF_Msk |
                         EMAC_MISTA_TXCP_Msk |
                         EMAC_MISTA_TXEMP_Msk |
                         EMAC_MISTA_TXINTR_Msk);
...
void EMAC_EnableTS(uint32_t u32Sec, uint32_t u32Nsec)
{
    double f;
    uint32_t reg;
    EMAC->TSCR = EMAC_TSCR_TSEN_Msk;
    EMAC->TSMSUR = u32Sec;   // Assume current time is 0 sec + 0 nano sec
    EMAC->TSLSUR = EMAC_Nsec2Subsec(u32Nsec);
    ...
    EMAC->TSIR = (reg = (uint32_t)f);
    f = (double)9223372036854775808.0 / ((double)(CLK_GetHCLKFreq()) * (double)reg);
    EMAC->TSAR = (uint32_t)f;
    EMAC->TSCR |= (EMAC_TSCR_TSFUEN_Msk | EMAC_TSCR_TSIEN_Msk);
}
...
void EMAC_GetTime(uint32_t *pu32Sec, uint32_t *pu32Nsec)
{
    ...
    *pu32Nsec = EMAC_Subsec2Nsec(EMAC->TSLSR);
    *pu32Sec = EMAC->TSMSR;
}
...
{
    // Disable time stamp update before update time value
    EMAC->TSCR = EMAC_TSCR_TSEN_Msk;
    EMAC->TSMSUR = u32Sec;
    EMAC->TSLSUR = EMAC_Nsec2Subsec(u32Nsec);
    EMAC->TSCR |= (EMAC_TSCR_TSFUEN_Msk | EMAC_TSCR_TSIEN_Msk);
...
void EMAC_EnableAlarm(uint32_t u32Sec, uint32_t u32Nsec)
{
    ...
    EMAC->TSMSAR = u32Sec;
    EMAC->TSLSAR = EMAC_Nsec2Subsec(u32Nsec);
    EMAC->TSCR |= EMAC_TSCR_TSALEN_Msk;
...
void EMAC_UpdateTime(uint32_t u32Neg, uint32_t u32Sec, uint32_t u32Nsec)
{
    ...
    EMAC->TSLSUR = EMAC_Nsec2Subsec(u32Nsec);
    if(u32Neg)
        EMAC->TSLSUR |= BIT31;   // Set bit 31 indicates this is a negative value

    EMAC->TSCR |= EMAC_TSCR_TSTUEN_Msk;
