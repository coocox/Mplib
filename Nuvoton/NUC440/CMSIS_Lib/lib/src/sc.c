 ...
 * $Revision: 2 $
 * $Date: 14/01/13 1:58p $
 * @brief    NUC400 series Smartcard(SC) driver source file
 *
 * @note
...
uint32_t SC_IsCardInserted(SC_T *sc)
{
    ...
    uint32_t cond1 = ((sc->SR & SC_SR_CD_PIN_ST_Msk) >> SC_SR_CD_PIN_ST_Pos);
    uint32_t cond2 = ((sc->CTL & SC_CTL_CD_LEV_Msk) >> SC_CTL_CD_LEV_Pos);
...
void SC_Open(SC_T *sc, uint32_t u32CD, uint32_t u32PWR)
{
    ...
    if(u32CD != SC_PIN_STATE_IGNORE) {
        u32Reg = u32CD ? 0: SC_CTL_CD_LEV_Msk;
    ...
    while(sc->CTL & SC_CTL_SYNC_Msk);
    sc->CTL = SC_CTL_SC_CEN_Msk | u32Reg;
}
...
void SC_ResetReader(SC_T *sc)
{
    ...
    sc->ALTCTL |= (SC_ALTCTL_TX_RST_Msk | SC_ALTCTL_RX_RST_Msk | SC_ALTCTL_ADAC_CD_EN_Msk);
