...
#define EN_RMCEN_SET                   ((uint32_t)0x00000001)
#define EN_RMCEN_CLEAR                 ((uint32_t)0xFFFFFFFE)
...
/**
  * @brief  Enable or disable RMC when system is in idle mode.
  * @param  RMCx: Select the RMC channel.
  *             This parameter can be one of the following values:
  *             TSB_RMC0, TSB_RMC1(M330 only)
  *         NewState: The new state of enable or disable
...
void RMC_SetIdleMode(TSB_RMC_TypeDef * RMCx, FunctionalState NewState)
{
    ...
    if (NewState == ENABLE) {
        /* Set RMCxEN<I2RMC> to enable RMCx running in IDLE */
        RMCx->EN |= EN_I2RMC_SET;
    } else {
        /* Clear RMCxEN<I2RMC> to disable RMCx running in IDLE */
        RMCx->EN &= EN_I2RMC_CLEAR;
    }
}
