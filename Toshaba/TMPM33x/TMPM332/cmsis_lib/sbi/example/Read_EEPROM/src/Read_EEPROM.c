...
/*---------------------------- Write EEPROM 2 bytes --------------------------*/
    ...
    gI2C_WCnt = 0U;
    do{
        i2c_state = SBI_GetI2CState(TSB_SBI1);
    } while (i2c_state.Bit.BusState);
    SBI_SetSendData(TSB_SBI1, SLAVEADDR | SBI_I2C_SEND);
    SBI_GenerateI2CStart(TSB_SBI1);
...
/*---------------------------- Read EEPROM 2 bytes ---------------------------*/
    ...
    SBI_SetSendData(TSB_SBI1, SLAVEADDR | SBI_I2C_RECEIVE);
    SBI_GenerateI2CStart(TSB_SBI1);
