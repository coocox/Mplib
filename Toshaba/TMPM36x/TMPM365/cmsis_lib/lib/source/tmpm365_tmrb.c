...
#define CR_FT0SEL_SET         ((uint32_t)0x00000010)
#define CR_FT0SEL_CLEAR       ((uint32_t)0xFFFFFFEF)
...
/**
  ...
  */
uint16_t TMRB_GetCaptureValue(TSB_TB_TypeDef * TBx, uint8_t CapRegNum)
{
    /* Check the parameters */
    assert_param(IS_TMRB_ALL_PERIPH(TBx));
    assert_param(IS_TMRB_CAPTURE_REG(CapRegNum));

    return CapRegNum ? (uint16_t) TBx->CP1 : (uint16_t) TBx->CP0;
}
...
#endif                          /* defined(__TMPM365_TMRB_H) */
