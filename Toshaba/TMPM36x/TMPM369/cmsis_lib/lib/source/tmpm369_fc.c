...
/* Includes ------------------------------------------------------------------*/
#include "tmpm369_fc.h"

#if defined(TSB_M369FD) || defined(TSB_M369FY)
...
#define FC_ERASE_CHIP_OVER_TIME             ((uint32_t)0x000FFFFF)      /* FC_ERASE_CHIP_OVER_TIME > 1.0s */
#define FC_ERASE_BLOCK_OVER_TIME            ((uint32_t)0x0001FFFF)      /* FC_ERASE_BLOCK_OVER_TIME > 0.1s */
...
/**
  ...
  *   This parameter can be one of the following values:
  *   FC_BLOCK_0(for TMPM369FD), FC_BLOCK_1(for TMPM369FD),
...
#endif
       (ENABLE == result2) && (ENABLE == result3) &&
       (ENABLE == result4) && (ENABLE == result5)) {
        retval = FC_ERROR_PROTECTED;
    } else {
        *addr1 = (uint32_t) 0x000000AA; /* bus cycle 1 */
        *addr2 = (uint32_t) 0x00000055; /* bus cycle 2 */
...
#endif                          /* defined(TSB_M369FD) || defined(TSB_M369FY) */
