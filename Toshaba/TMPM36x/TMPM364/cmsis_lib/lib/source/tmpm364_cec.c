...
/** @defgroup CEC_Private_Defines
  * @{
  */
#define EN_CECEN_SET                          ((uint32_t)0x00000003)
#define EN_CECEN_CLEAR                        ((uint32_t)0x00000002)
...
Result CEC_SetRxStartBitWaveConfig(CEC_StartBitRisingTimeMin RisingTimeMin,
                                   ...
                                   CEC_StartBitCycleMin CycleMin, CEC_StartBitCycleMax CycleMax)
{
    Result retval = ERROR;
    uint32_t tmp = 0U;
...
/**
  * @brief Specify the sampling clock for CEC function
  * @param  ClkSrc: the sampling clock Source
  *   This parameter can be:
  *   CEC_CLK_SRC_32KHZ or CEC_CLK_SRC_TBAOUT
  * @retval None
  */
void CEC_SetSrcClk(CEC_SrcClk ClkSrc)
{
    assert_param(IS_CEC_CLK_SRC(ClkSrc));
    if (ClkSrc == CEC_CLK_SRC_32KHZ) {
        TSB_CEC_FSSEL_CECCLK = 0U;
    } else {
        TSB_CEC_FSSEL_CECCLK = 1U;
    }
}
