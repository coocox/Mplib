...
#define CG_RESET_FLAG_MASK          ((uint32_t)0xFFFFFFE2)
...
#define FPLL_MULTIPLY_8_VALUE       ((uint32_t)0x0000703C)
#define FPLL_MULTIPLY_6_VALUE       ((uint32_t)0x0000702C)
...
/**
  ...
  *   This parameter can be one of the following values:
  *   CG_FPLL_MULTIPLY_8 or CG_FPLL_MULTIPLY_6
  * @retval Success or not
  *   The value returned can be one of the following values:
  *   SUCCESS or ERROR
  */
Result CG_SetFPLLValue(CG_FpllValue NewValue)
{
    ...
    if ((fosc_src == CG_FOSC_OSC_INT) || (pllon == ENABLE)) {
        ...
        if (NewValue == CG_FPLL_MULTIPLY_8) {
            ...
            if (NewValue == CG_FPLL_MULTIPLY_6) {
                TSB_CG->PLLSEL = FPLL_MULTIPLY_6_VALUE;
                retval = SUCCESS;
            } else {
                retval = ERROR;
            }
...
CG_FpllValue CG_GetFPLLValue(void)
{
    ...
    case FPLL_MULTIPLY_8_VALUE:
        ...
        break;
    case FPLL_MULTIPLY_6_VALUE:
        retval = CG_FPLL_MULTIPLY_6;
        break;
    default:
        /* Do nothing */
        break;
...
{
    Result retval = ERROR;
    CG_FcSrc fcsrc = CG_FC_SRC_HALF_FPLL;
    ...
    if (NewState == ENABLE) {
        ...
        if (fcsrc == CG_FC_SRC_FOSC) {  /* PLL is not used. So it can be shut off */
            ...
            retval = SUCCESS;
        } else {                /*fcsrc == CG_FC_SRC_HALF_FPLL. PLL is in use, so disabling it should be invalid */
            /* Do nothing */
        }
    }
...
/**
  ...
  *   This parameter can be one of the following values:
  *   CG_FC_SRC_FOSC or CG_FC_SRC_HALF_FPLL
  * @retval success or failure
  *   The value returned can be one of the following values:
  *   SUCCESS or ERROR
  */
...
/**
  * @brief  Set the source of USB device block
  * @param  Source: Select source clock to USB device block
  *   This parameter can be one of the following values:
  *   CG_USB_SRC_CLK_PLL ,or CG_USB_SRC_CLK_EXT 
  * @retval None
  */
void CG_SetUSBSrcClk(CG_USBSrc Source)
{
    /* Check the parameters */
    assert_param(IS_CG_USB_SRC(Source));

    if (Source == CG_USB_SRC_CLK_EXT) {
        TSB_CG_USBCTL_USBCLKSEL = 1U;
    } else {
...
/**
  * @brief  Get the source of USB device block
  * @param  None
  * @retval Source of USB device block.
  *   The value returned can be one of the following values:
  *   CG_USB_SRC_CLK_PLL ,or CG_USB_SRC_CLK_EXT 
  */
CG_USBSrc CG_GetUSBSrcClk(void)
{
    CG_USBSrc source = CG_USB_SRC_CLK_PLL;

    if (TSB_CG_USBCTL_USBCLKSEL == 1U) {
        source = CG_USB_SRC_CLK_EXT;
    } else {
        /* Do nothing */
    }
...
/**
  * @brief  Set the state of USB source clock
  * @param  NewState: NewState of USB source clock
...
void CG_SetUSBClkState(FunctionalState NewState)
{
    ...
    if ( NewState == ENABLE ) {
        TSB_CG_USBCTL_USBCLKEN=1U;
    } else {
        TSB_CG_USBCTL_USBCLKEN=0U;
    }
}

/**
  * @brief  Get the state of USB souce clock
  * @param  None
  * @retval state of USB source clock control
  *   The value returned can be one of the following values:
  *   ENABLE or DISABLE
  */
