...
#define DMACA_SOFTBREQ_MASK          ((uint32_t)0x0000CFFF)
#define DMACB_SOFTBREQ_MASK          ((uint32_t)0x0000FFFF)
#define DMACA_SOFTSREQ_MASK          ((uint32_t)0x0000C000)
#define DMACB_SOFTSREQ_MASK          ((uint32_t)0x0000FC00)
...
/**
  ...
  *   DMACA_SIO0_UART0_RX, DMACA_SIO0_UART0_TX, DMACA_SIO1_UART1_RX, DMACA_SIO1_UART1_TX, 
  *   DMACA_TMRB8_CMP_MATCH, DMACA_TMRB9_CMP_MATCH, DMACA_TMRB0_CAPTURE0, DMACA_TMRB4_CAPTURE0, 
  *   DMACA_TMRB4_CAPTURE1, DMACA_TMRB5_CAPTURE0, DMACA_TMRB5_CAPTURE1, DMACA_TOP_PRIORITY_ADC,
  *   DMACA_FULL_UART_RX, DMACA_FULL_UART_TX
  * @retval None
  */
void DMACA_SetSWBurstReq(DMACA_ReqNum BurstReq)
{
    ...
    tmp &= ~mask;
    tmp &= (uint32_t) DMACA_SOFTBREQ_MASK;
    mask = 1U;
...
/**
  ...
  *   DMACB_TMRB6_CAPTURE1, DMACB_NORMAL_ADC, DMACB_SSP0_TX, DMACB_SSP0_RX, 
  *   DMACB_SSP1_TX, DMACB_SSP1_RX, DMACB_SSP2_TX, DMACB_SSP2_RX
  * @retval None
  */
void DMACB_SetSWBurstReq(DMACB_ReqNum BurstReq)
{
    ...
    mask <<= ((uint32_t) BurstReq);
    tmp &= ~mask;
    tmp &= (uint32_t) DMACB_SOFTBREQ_MASK;
    mask = 1U;
    tmp |= (uint32_t) (mask << BurstReq);
    /* Set DMA software burst request */
...
DMAC_BurstReqState DMAC_GetSWBurstReqState(TSB_DMAC_TypeDef * DMACx)
{
    ...
    if (TSB_DMACA == DMACx) {
        tmp.All &= DMACA_SOFTBREQ_MASK;
    } else {
        tmp.All &= DMACB_SOFTBREQ_MASK;
    }
...
void DMACA_SetSWSingleReq(DMACA_ReqNum SingleReq)
{
    ...
    /* Check the parameters */
    assert_param(IS_DMACA_SINGLE_REQ_NUM(SingleReq));

    tmp = TSB_DMACA->SOFTSREQ;
    mask <<= ((uint32_t) SingleReq);
    tmp &= ~mask;
    tmp &= (uint32_t) DMACA_SOFTSREQ_MASK;
    mask = 1U;
    tmp |= (uint32_t) mask << SingleReq;
    /* Set DMA software single request */
    TSB_DMACA->SOFTSREQ = tmp;
}
...
void DMACB_SetSWSingleReq(DMACB_ReqNum SingleReq)
{
    ...
    mask <<= ((uint32_t) SingleReq);
    tmp &= ~mask;
    tmp &= (uint32_t) DMACB_SOFTSREQ_MASK;
    mask = 1U;
    tmp |= (uint32_t) mask << SingleReq;
    /* Set DMA software single request */
...
DMAC_SingleReqState DMAC_GetSWSingleReqState(TSB_DMAC_TypeDef * DMACx)
{
    ...
    if (TSB_DMACA == DMACx) {
        tmp.All &= DMACA_SOFTSREQ_MASK;
    } else {
        tmp.All &= DMACB_SOFTSREQ_MASK;
    }
